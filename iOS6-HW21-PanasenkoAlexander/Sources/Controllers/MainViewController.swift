...
class MainViewController: UIViewController {
    ...
    var isColor: Bool = false {
        didSet {
            if isColor {
                self.mainView!.backgroundColor = .link
            } else {
                self.mainView!.backgroundColor = .systemGreen
            }
        }
    }
    ...
    @objc func buttonTapped() {
        bruteForce(passwordToUnlock: mainView?.currentPasswordTextField.text ?? "ошибка")
    ...
    func bruteForce(passwordToUnlock: String) {
        ...
        let queue = DispatchQueue(label: "подбор", qos: .utility, attributes: .concurrent)
        queue.async {
            if self.parsingIsActive == false {
                while password != passwordToUnlock {
                    if self.parsingIsActive == false {
                        ...
                        DispatchQueue.main.sync {
                            self.mainView?.descriptionLabel.text = "Пароль: - \(password)"
                            self.mainView?.passwordActionButton.setTitle("Отмена подбора пароля", for: .normal)
                            self.mainView?.activityIndicator.startAnimating()
                            if password == passwordToUnlock {
                                self.mainView?.passwordActionButton.setTitle("Начать подбор пароля", for: .normal)
                ...
                DispatchQueue.main.sync {
                    self.mainView?.passwordActionButton.setTitle("Начать подбор пароля", for: .normal)
                    self.mainView?.descriptionLabel.text = "Пароль: - \(self.mainView?.currentPasswordTextField.text ?? "Случайный подбор") не взломан"
                    self.mainView?.activityIndicator.stopAnimating()
                }
            }
        }
